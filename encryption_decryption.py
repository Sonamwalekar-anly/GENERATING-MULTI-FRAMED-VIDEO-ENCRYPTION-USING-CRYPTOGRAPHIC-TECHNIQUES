...
window.title("Main Window 1") # title given is "DICTIONARY"
window.geometry('1000x700')

# top label
start1 = tk.Label(text = "Splash Screen", font=("Arial", 55,"underline"), fg="Black") # same way bg
...
window1.title("Main Window Screen") # title given is "DICTIONARY"
...
def encrypt_fun():
    ...
    ic_list = []
    for i in range(x2):
        ic_list.append(ImageClip("Images/sample.jpg").with_duration(1))
    video = concatenate_videoclips(ic_list, method="compose")
    ...
    while True:
        ...
        cv2.imshow("Encrypted Form", img1)
        # exiting the loop
        key = cv2.waitKey(1)
        if key == ord("q"):
            break
...
def decrypt_fun():
    ...
    while True:
        ...
        # displaying the video
        cv2.imshow("Decrypted Form", gray)
        # exiting the loop
        key = cv2.waitKey(1)
        if key == ord("q"):
            break
...
def reset_fun():
    ...
    while True:
        ...
        # displaying the video
        cv2.imshow("Original Content", img3)
        # exiting the loop
        key = cv2.waitKey(1)
        if key == ord("q"):
            break
...
# top label
start1 = tk.Label(text = "ENCRYPTION\nDECRYPTION", font=("Arial", 55, "underline"), fg="black") # same way bg
start1.place(x = 120, y = 10)

# lbl1 = tk.Label(text="Select any video, dimension & crop it...", font=("Arial", 40),fg="green")  # same way bg
# lbl1.place(x=50, y=100)

lbl2 = tk.Label(text="Selected", font=("Arial", 30),fg="brown")  # same way bg
...
# Select Button
selectb=Button(window1, text="ENCRYPT",command=encrypt_fun,  font=("Arial", 25), bg = "orange", fg = "blue")
selectb.place(x = 120, y = 450)

# Select Button
selectb=Button(window1, text="DECRYPT",command=decrypt_fun,  font=("Arial", 25), bg = "orange", fg = "blue")
